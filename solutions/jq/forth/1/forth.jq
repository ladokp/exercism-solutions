...
def new_command(cmd):
  if (cmd[0] | isnumber) then "illegal operation" | halt_error
    ...
    .cmds as $c | .cmds[cmd[0]] = reduce cmd[1:-1][] as $word ([];
        . + if ($c | has($word)) then $c[$word] else [$word] end
      )
  end;

def need(n):
  (.stack | length) as $len | if   $len == 0 and n > 0 then "empty stack" | halt_error
...
def divide:
  need(2) | if .stack[-1] == 0 then "divide by zero" | halt_error
...
reduce .instructions[] as $line (
  {stack: [], cmds: {}};
  evaluate($line | ascii_downcase | split("\\s+";""))) | .stack