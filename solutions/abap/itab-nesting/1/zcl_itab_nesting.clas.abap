...
CLASS zcl_itab_nesting IMPLEMENTATION.
  ...
  METHOD perform_nesting.
     nested_data = VALUE #( FOR ls_artists IN artists (
        artist_id = ls_artists-artist_id artist_name = ls_artists-artist_name albums =
        VALUE #( FOR ls_album IN albums WHERE ( artist_id = ls_artists-artist_id ) (
                    album_id = ls_album-album_id album_name = ls_album-album_name songs =
                    VALUE #( FOR ls_songs IN songs WHERE ( artist_id = ls_album-artist_id AND album_id = ls_album-album_id ) (
                               song_id = ls_songs-song_id song_name = ls_songs-song_name ) ) ) ) ) ).
